<?php
	...
	//get number of rows
	$queryNum = $db_connect->query("SELECT COUNT(*) as postNum FROM sharp_emp WHERE status = 'Ex empleado'");
	...
	<section class="contentSection right clearfix">
		<div class="container">
			<div class="wrapper employee_list clearfix">
				<div class="section_title">Empleados anteriores</div>
				<div class="top-bar">
					<div class="top-item">
						<form id="empFilter" method="post" action="">
							<input class="filterField filterVal" type="text" placeholder="Buscar" onkeyup="pastsearchFilter()">
						</form>
					</div>
					<div class="top-item">
						<form id="empFilter" method="post" action="">
				...
				<?php
					$getemp = mysqli_query($db_connect, "SELECT * FROM sharp_emp WHERE status = 'Ex empleado' ORDER BY id ASC");
					...
					<div id="displayempList">
						<?php
							if($getempcount >= 1 ){
								...
								echo '<li class="emp_item"> No se encontró ningún registro de empleado </li>';
							}
						?>
					</div>
				</ul>
